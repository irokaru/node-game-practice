export default class Validator {
  ...
  errors(err = {}) {
    for (let [key, rule] of Object.entries(this._rules)) {
      ...
      // check type
      if (type !== 'callback' && !this.$._validateType[type](value)) {
      ...
      if (Validator.hasKeyInObject(rule, 'min')) {
        if (!this.$._validateMin[type](value, rule.min)) {
      ...
      if (Validator.hasKeyInObject(rule, 'max')) {
        if (!this.$._validateMax[type](value, rule.max)) {
      ...
      if (Validator.hasKeyInObject(rule, 'pattern') && type === 'string') {
        if (!this.$._strPatterns[rule.pattern](value)) {
